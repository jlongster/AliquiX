...
AXProfileSample::AXProfileSample(string SampleName) {
	...
	for(int i=0; i<MAX_PROFILER_SAMPLES; i++) {
		if(!_ProfileSamples[i].IsValid) {
			...
			_ProfileSamples[i].StartTime = AXTaskClock::GetTime();
...
void AXProfileSample::Output() {
	...
	AXAppLog->Write(MESSAGE_PROFILE, "\nAliquiX Profile Sample (%2.5f seconds)", _TimeRootEnd-_TimeRootBegin);
	AXAppLog->Write(MESSAGE_PROFILE, "pMin\t| pAvg\t| pMax\t| cMin\t| cAvg\t| cMax\t| Name");
	AXAppLog->Write(MESSAGE_PROFILE, "-----------------------------------------------------------------");
	...
	for(int i=0; i<MAX_PROFILER_SAMPLES; i++) {
		...
		AXAppLog->Write(MESSAGE_PROFILE, "%3.1f\t| %3.1f\t| %3.1f\t| %3.1f\t| %3.1f\t| %3.1f\t| %s",
...
void AXProfileSample::ResetSample(string Name) {
	for(int i=0; i<MAX_PROFILER_SAMPLES; i++) {
